<template>
  <div class="space-y-5">
    ...
    <!-- Import Modal -->
    <BaseModal v-model="showImportModal" :title="$t('brands.importTitle')" size="md">
      <div class="space-y-4">
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ $t('brands.importDesc') }}
        ...
        <div v-if="importResult" class="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
          {{ importResult.message }} &mdash; {{ importResult.created }} {{ $t('brands.importCreated') }}, {{ importResult.updated }} {{ $t('brands.importUpdated') }}.
        </div>
...
const form = reactive({ br_title: '', br_status: true })
...
async function onImportFile(e: Event) {
  ...
  if (!file) return
  await importExcel('/import/brands', file)
