...
const incrStore = useDocumentIncrementorStore()
...
async function fetchNextReference(incrementorId: string | number) {
  if (!incrementorId) {
...
const filteredIncrementors = computed(() => incrStore.items.filter((i) => i.di_model === header.document_type))

const destWarehouses = computed(() => whStore.items.filter((wh) => String(wh.id) !== String(header.warehouse_id)))

// ── Auto-select incrementor + fetch reference on type change ──────────────
watch(
...
onMounted(async () => {
  ...
  if (!incrStore.items.length) await incrStore.fetchAll()
  const first = filteredIncrementors.value[0]
  if (first) {
    header.document_incrementor_id = first.id
    fetchNextReference(first.id)
  }
  const res = await http.get<{ data: Product[] }>('/products', { params: { per_page: 500 } })
  products.value = res.data?.data ?? (res.data as unknown as Product[])
})
...
async function submit() {
  ...
  if (!header.document_incrementor_id) {
    errorMsg.value = 'Veuillez sélectionner un numéroteur.'
    return
  }
  ...
  try {
    const payload = {
      ...
      document_incrementor_id: header.document_incrementor_id,
    ...
    errorMsg.value = err.response?.data?.message ?? 'Erreur lors de la création.'
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="max-w-9xl mx-auto py-6 px-4">
    <!-- Page Header -->
    <div class="mb-6">
      <router-link to="/stock/documents" class="text-sm text-violet-600 dark:text-violet-400 hover:underline">
        &larr; Retour aux documents de stock
      </router-link>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white mt-2">
        Nouveau {{ $t(`stock.${header.document_type}`) }}
      </h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
        Créez une entrée, sortie, ajustement ou transfert de stock.
      </p>
    </div>
    ...
    <form class="space-y-6" @submit.prevent="submit">
      ...
      >
        ...
        >
          ...
          <span
            v-else-if="nextReference"
            class="px-3 py-0.5 text-lg font-mono font-semibold text-gray-800 dark:text-gray-200"
          >
            Numéro: {{ nextReference }}
          </span>
          <span
            v-else-if="header.document_incrementor_id === ''"
            class="px-2.5 py-0.5 text-xs bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-full border border-amber-200 dark:border-amber-700"
          >
            Aucun numéroteur
          </span>
        </div>
      ...
      <div class="flex items-center justify-between">
        <button
          ...
          @click="router.push('/stock/documents')"
        >
          Annuler
        </button>
        <button
