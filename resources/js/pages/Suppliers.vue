<template>
  <div class="space-y-5">
              ...
              <tfoot>
                <tr class="border-t-2 border-gray-200 dark:border-gray-700 font-semibold bg-gray-50 dark:bg-gray-900">
                  <td colspan="3" class="py-2.5 px-3 text-sm text-gray-600 dark:text-gray-400">
                    {{ supplierDocuments.length }} document(s)
                  </td>
    ...
    <BaseModal
      ...
      :title="'Enregistrer un paiement — ' + (bulkPaymentTarget?.tp_title ?? '')"
      size="md"
    >
      <div class="space-y-5">
        ...
        <div v-else-if="!bulkPaymentLoading" class="text-center py-8 text-gray-400 dark:text-gray-500">
          <svg
        ...
        <div v-if="bulkPaymentUnpaidDocs.length > 0 && !bulkPaymentLoading" class="space-y-4">
          ...
          <!-- Payment result -->
          <div
            v-if="bulkPaymentResult"
            class="rounded-lg border p-3 text-sm"
            :class="
              bulkPaymentResult.remaining > 0
                ? 'bg-amber-50 border-amber-200 text-amber-800'
                : 'bg-emerald-50 border-emerald-200 text-emerald-800'
            "
          >
            <p class="font-semibold">{{ bulkPaymentResult.message }}</p>
            <p class="text-xs mt-1">Montant affecté : {{ formatNumber(bulkPaymentResult.total_applied) }} DH</p>
            <p v-if="bulkPaymentResult.remaining > 0" class="text-xs">
              Excédent non affecté : {{ formatNumber(bulkPaymentResult.remaining) }} DH
            </p>
          </div>
        </div>
      </div>
      <template #footer>
        ...
        >
          ...
          {{ bulkPaymentSaving ? 'Enregistrement...' : 'Enregistrer le paiement' }}
        </button>
      </template>
    </BaseModal>
    ...
    <!-- Import Modal -->
    <BaseModal v-model="showImportModal" title="Import fournisseurs" size="md">
      <div class="space-y-4">
        <p class="text-sm text-gray-600 dark:text-gray-400">
...
const totalDocsTTC = computed(() =>
  supplierDocuments.value.reduce((sum: number, inv: any) => sum + Number(inv.footer?.total_ttc ?? 0), 0),
)

const totalDocsDue = computed(() =>
  supplierDocuments.value.reduce((sum: number, inv: any) => sum + Number(inv.footer?.amount_due ?? 0), 0),
)
