...
const canConvert = computed(() => {
  ...
  if (doc.value.document_type === 'PurchaseOrder' && doc.value.status === 'confirmed') return true
  if (doc.value.document_type === 'ReceiptNotePurchase' && doc.value.status === 'confirmed') return true
  return false
})
const canPay = computed(() => {
  if (!doc.value) return false
  return (
    doc.value.document_type === 'InvoicePurchase' &&
...
const canReturn = computed(() => {
  if (!doc.value) return false
  return (
    ['ReceiptNotePurchase', 'InvoicePurchase'].includes(doc.value.document_type) &&
    ['confirmed', 'received', 'pending', 'paid', 'partial'].includes(doc.value.status)
  )
})
const canDelete = computed(() => doc.value?.status === 'draft')

const convertLabel = computed(() => {
  if (doc.value?.document_type === 'PurchaseOrder') return 'Générer Bon de Réception'
  if (doc.value?.document_type === 'ReceiptNotePurchase') return 'Générer Facture Achat'
  return 'Convertir'
})
...
async function convertDocument() {
  ...
  // For ReceiptNote → PurchaseInvoice, show payment method selection modal first
  if (doc.value.document_type === 'ReceiptNotePurchase') {
...
async function createReturn() {
  ...
  actionLoading.value = true
  try {
    const result = await store.retourFournisseur(doc.value.id)
    if (result.success && result.retour) {
      toast.success(`Retour Fournisseur ${result.retour.reference} créé. Stock mis à jour.`)
      router.push(`/achats/documents/${result.retour.id}`)
      return
    }
  } catch { /* Axios interceptor shows toast */ }
  actionLoading.value = false
...
<template>
  <div class="max-w-9xl mx-auto py-4 sm:py-6 px-3 sm:px-4">
    ...
    <div v-else-if="doc">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          <router-link
            to="/achats/documents"
            class="text-sm text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-300 transition"
          >
            &larr; Documents d'Achat
          </router-link>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-4 mb-6">
        <div class="flex flex-wrap items-center gap-2">
          <router-link
            v-if="canEdit"
            :to="`/achats/documents/${doc.id}/edit`"
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          ...
          <button
            type="button"
            class="text-teal-600 hover:text-teal-700 hover:underline transition font-semibold text-sm"
          ...
          <div v-if="doc.footer" class="space-y-4">
            ...
            <div class="flex justify-between text-sm">
              ...
              <span
                ...
                "
                >{{ fmt(doc.footer.amount_due) }} DH</span
              >
            </div>
            <div class="relative pt-1">
              ...
              <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div
                  ...
                  :class="paymentProgress >= 100 ? 'bg-emerald-500' : 'bg-teal-500'"
      ...
      >
        ...
        <div class="flex flex-wrap gap-2">
          <router-link
            v-if="doc.parent"
            :to="`/achats/documents/${doc.parent.id}`"
            class="inline-flex items-center gap-1.5 text-sm text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 hover:bg-teal-100 dark:hover:bg-teal-900/50 px-3 py-1.5 rounded-lg transition"
          >
    ...
    <PaymentModal
      ...
      :label="`${typeLabels[doc?.document_type ?? ''] ?? ''} : ${doc?.reference ?? ''} — ${doc?.third_partner?.tp_title ?? doc?.thirdPartner?.tp_title ?? ''}`"
    ...
    <BaseModal v-model="showInvoiceModal" title="Générer la Facture Achat" size="sm">
      <div class="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3.5 py-2.5 text-input rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
          >
        ...
        >
          ...
          <p class="text-xs text-amber-700 dark:text-amber-300">
            Le montant TTC sera ajouté à l'encours du fournisseur.
          </p>
        </div>
      </div>
      <template #footer>
    ...
    >
      ...
      les marchandises sortent du stock et repartent au fournisseur. Continuer ?
    </ConfirmModal>
