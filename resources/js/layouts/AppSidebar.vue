<template>
  ...
  >
    ...
    <div class="flex items-center gap-3 px-3 h-16 border-b border-slate-800 shrink-0 overflow-hidden">
      <div
        class="w-9 h-9 rounded-xl bg-[#7C5CFC] flex items-center justify-center shrink-0 shadow-lg shadow-[#7C5CFC]/40"
      >
    ...
    <div class="shrink-0 border-t border-slate-800 p-3 flex items-center gap-3 overflow-hidden">
      <div
        class="w-9 h-9 rounded-full bg-[#7C5CFC] flex items-center justify-center shrink-0 text-white text-xs font-bold uppercase"
      >
        {{ userInitials }}
      </div>
      <Transition
...
const isExpanded = computed(() => props.pinned || hovered.value)
...
const icons = {
  ...
  productsTrashed: `<svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>`,
...
const tenantNavItems = computed((): SidebarLink[] => {
  const items: SidebarLink[] = [
    ...
    { to: '/products/trashed', labelKey: 'nav.productsTrashed', icon: icons.productsTrashed, groupKey_ref: 'nav.catalogue' },
