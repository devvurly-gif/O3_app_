...
class Product extends Model
{
    ...
    protected static function booted(): void
    {
        static::creating(function (self $product) {
            ...
            // Strategy:
            //   1) reuse p_code (already generated by BelongsToStructure trait,
            //      which fires first — guaranteed unique via incrementor),
            //   2) fall back to a title-slug + random suffix if p_code missing,
            //   3) in all cases, add `-1`, `-2`, … on collision with an existing row.
            if (empty($product->p_sku)) {
                $base = !empty($product->p_code)
                    ? $product->p_code
                    : 'SKU-' . Str::upper(Str::random(6));

                $candidate = $base;
                $suffix = 1;
                while (static::where('p_sku', $candidate)->exists()) {
                    $candidate = $base . '-' . $suffix;
                    $suffix++;
                }
                $product->p_sku = $candidate;
            }
        });
