...
sheet[f'B{row}'] = 'Mme Amina Ouzzine'
sheet[f'A{row}'].font = Font(bold=True, size=11)
sheet[f'B{row}'].font = Font(bold=True, size=11)
row += 1
...
section_titles = {
    'fournitures': '1. FOURNITURES - MEUBLES ET EQUIPEMENTS',
    'revetements': '2. REVETEMENTS - SOL ET MURS',
    'travaux': '3. TRAVAUX D INSTALLATION',
    'electro': '4. ELECTRICITE ET PLOMBERIE',
    'finitions': '5. FINITIONS ET SERVICES'
}

subtotal_rows = {}

for section_key in ['fournitures', 'revetements', 'travaux', 'electro', 'finitions']:

    # Add section header (skip for first since already added)
    if section_key != 'fournitures':
        sheet[f'A{row}'] = section_titles[section_key]
        sheet[f'A{row}'].font = section_font
        sheet[f'A{row}'].fill = section_fill
        for col in range(1, 5):
            sheet.cell(row=row, column=col).fill = section_fill
    ...
    for designation, qty, price in data_sections[section_key]:
    ...
    data_end = row - 1
    subtotal_rows[section_key] = row

    sheet[f'A{row}'] = f'Sous-total {section_key.capitalize()}'
    sheet[f'A{row}'].font = Font(bold=True, size=10, italic=True)
    sheet[f'D{row}'] = f'=SUM(D{data_start}:D{data_end})'
...
formula_parts = []
for key in ['fournitures', 'revetements', 'travaux', 'electro', 'finitions']:
    formula_parts.append(f'D{subtotal_rows[key]}')

sheet[f'D{row}'] = '=' + '+'.join(formula_parts)
...
wb.save('Devis_Amina_Ouzzine_Cuisine_200K_DH.xlsx')
print("Devis cree avec nom du client!")
